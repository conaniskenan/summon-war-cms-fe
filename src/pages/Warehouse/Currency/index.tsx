 ...
 * @LastEditTime: 2024-04-06 21:21:52
...
const Currency: React.FC = () => {
  ...
  const { run: getCurrencyListRun } = useRequest(getCurrencyList, {
    manual: true,
    onSuccess: (res) => {
      setList(res.list)
    }
  })
  const { run: postCurrencyModifyRun } = useRequest(postCurrencyModify, {
    manual: true,
    onSuccess: () => {
  ...
  const { run: postCurrencyDeleteRun } = useRequest(postCurrencyDelete, {
    manual: true,
    onSuccess: () => {
      setRefresh((prev) => prev + 1)
      notification.success({
  ...
  const { run: postCurrencyDefaultRun, loading: postCurrencyDefaultLoading } =
    useRequest(postCurrencyDefault, {
      manual: true,
      onSuccess: () => {
        setRefresh((prev) => prev + 1)
  ...
  const { run: postCurrencyReleaseRun, loading: postCurrencyReleaseLoading } =
    useRequest(postCurrencyRelease, {
      manual: true,
      onSuccess: () => {
        setRefresh((prev) => prev + 1)
  ...
  const handleSave = (
    row: Api.Paths.GetCurrencyList.Response['list'][0],
    preRow: Api.Paths.GetCurrencyList.Response['list'][0]
  ) => {
  ...
  const columns = [
    ...
    //操作
    {
      ...
      render: (
        _text: string,
        record: Api.Paths.GetCurrencyList.Response['list'][0]
      ) => {
        return (
          <div>
            <Button
  ...
  useEffect(() => {
    getCurrencyListRun({ version_id: 0 })
  }, [refresh])
